...
class FilamentSensorsRevolutions(octoprint.plugin.StartupPlugin,
   ...
   @octoprint.plugin.BlueprintPlugin.route("/filamentone", methods=["GET"])
    def api_get_filamentone(self):
        ...
        if self.runoutone_sensor_enabled():
            status = "0" if self.no_filamentone() else "1"
        return jsonify(status=status)
   ...
   @property
    ...
    def _setup_sensor(self):
        if self.runout_sensor_enabled() or self.jam_sensor_enabled() or self.runoutone_sensor_enabled():
            if self.mode == 0:
	   ...
	   if self.runoutone_sensor_enabled():
                ...
                self._logger.info("Runoutone Sensor Pin not configured")
            ...
            self._logger.info(  "Pins not configured, won't work unless configured!")
   ...
   def runoutone_sensor_enabled(self):
	...
	if self.runoutone_sensor_enabled() and self.no_filamentone():
	   ...
	   if self.runouotone_sensor_enabled():
                self._logger.info(
                    "%s: Enabling filament runout sensor." % (event))
                self.runoutone_triggered = 0  # reset triggered state
                GPIO.remove_event_detect(self.runoutone_pin)
                GPIO.add_event_detect(
                    self.runoutone_pin, GPIO.BOTH,
                    callback=self.runoutone_sensor_callback,
                    bouncetime=self.runoutone_bounce
                )
    ...
    def runout_sensor_callback(self, _):
        ...
        if self.no_filamentone():
            # Set the triggered flag to check next callback
            self.runoutone_triggered = 1
            self._logger.info("Out of filament!")
            if self.send_gcode_only_once:
                self._logger.info("Sending GCODE only once...")
            else:
            ...
            if self.runoutone_pause_print:
                self._logger.info("Pausing print.")
                self._printer.pause_print()
            if self.no_filamentone_gcode:
                self._logger.info("Sending out of filamentone GCODE")
                self._printer.commands(self.no_filamentone_gcode)
        else:
